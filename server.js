...
connectionLimit : 10,
host: 'rengok6m.beget.tech',
user: 'rengok6m_kuznica',
password: 'Q1qqqqqq',
database: 'rengok6m_kuznica'
});

console.log('Connected to the database');
app.get('/Posetitel/index/:id', (req, res) => {
    let query = 'SELECT * FROM posetitelkuz WHERE id = ?';
...
db.query('SELECT * FROM posetitelkuz', (err, results) => {
if (err) throw err;
res.json(results);
});
});
...
app.get('/Rabotnik/index/:id', (req, res) => {
    let query = 'SELECT * FROM rabotnikkuz WHERE id = ?';
    db.query(query, req.params.id, (err, result) => {
    if (err) {
    console.error(err);
    res.status(500).send('Server error: ' + err.message);
...
db.query('SELECT * FROM rabotnikkuz', (err, results) => {
if (err) throw err;
res.json(results);
});
});
...
app.get('/populaterab', (req, res) => {
    ...
    let query = 'INSERT INTO rabotnikkuz SET ?';
...
app.get('/populate', (req, res) => {
    ...
    Отчество: fields[2],
    Возраст: fields[3],
    Сумма_заказа: fields[4],
    Количество_товаров: fields[5],
    Тип_заказа: fields[6],
    Материал: fields[7],
    Способ_оплаты: fields[8],
    Фото: '/save/posetitel/' + getRandomPhoto()
    };
    let query = 'INSERT INTO posetitelkuz SET ?';
    db.query(query, item, (err, result) => {
    if (err) {
    console.error(err);
    res.status(500).send('Server error: ' + err.message);
...
app.post('/save/rabotnik/', uploadrab.single('file'), (req, res) => {
    ...
    app.put('/Posetitel/index/:id', upload.single('file'), (req, res) => {
        ...
        let query = 'UPDATE posetitelkuz SET ? WHERE id = ?';
        ...
        app.put('/Rabotnik/index/:id', upload.single('file'), (req, res) => {
            ...
            let query = 'UPDATE rabotnikkuz SET ? WHERE id = ?';
            db.query(query, [updatedItem, req.params.id], (err, result) => {
            if (err) {
            console.error(err);
            res.status(500).send('Server error: ' + err.message);
...
let query = 'DELETE FROM posetitelkuz WHERE id = ?';
db.query(query, req.params.id, (err, result) => {
if (err) throw err;
...
app.delete('/Rabotnik/index/:id', (req, res) => {
    let query = 'DELETE FROM rabotnikkuz WHERE id = ?';
    db.query(query, req.params.id, (err, result) => {
    if (err) throw err;
    res.send('Item deleted successfully');
    });
